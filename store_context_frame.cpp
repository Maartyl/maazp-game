...
store_context_frame::store_context_frame(dict_map&& d) : store_context_frame(store::transient<dict>(std::move(d))) { }
store_context_frame::store_context_frame(store::handle d) : dict_(d) {
  if (!store::deref(d).as_dict()) //just test
...
store::handle store_context_frame::find(store::id id) {
  if (REF d = store::deref(dict_).as_dict()) {
    if (auto h = store::dict_get(d, id))
      return h;
  } else throw std::invalid_argument("context_frame.find: requires dict handle");
  if (next_lvl)
